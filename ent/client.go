...
// QueryStore queries the store edge of a Price.
func (c *PriceClient) QueryStore(pr *Price) *StoreQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, price.StoreTable, price.StoreColumn),
		)
		fromV = sqlgraph.Neighbors(pr.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryStores queries the stores edge of a Product.
func (c *ProductClient) QueryStores(pr *Product) *StoreQuery {
	query := (&StoreClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := pr.ID
		step := sqlgraph.NewStep(
...
// QueryStore queries the store edge of a Review.
func (c *ReviewClient) QueryStore(r *Review) *StoreQuery {
	query := (&StoreClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := r.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(review.Table, review.FieldID, id),
			sqlgraph.To(store.Table, store.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, review.StoreTable, review.StoreColumn),
		)
		fromV = sqlgraph.Neighbors(r.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryPrices queries the prices edge of a Store.
func (c *StoreClient) QueryPrices(s *Store) *PriceQuery {
	query := (&PriceClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := s.ID
		step := sqlgraph.NewStep(
...
// QueryReviews queries the reviews edge of a Store.
func (c *StoreClient) QueryReviews(s *Store) *ReviewQuery {
...
type (
	hooks struct {
		Price, Product, Review, Store, User []ent.Hook
	}
	inters struct {
		Price, Product, Review, Store, User []ent.Interceptor
	}
)
