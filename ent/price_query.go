...
func (pq *PriceQuery) QueryStore() *StoreQuery {
...
func (pq *PriceQuery) Clone() *PriceQuery {
	...
	return &PriceQuery{
		...
		withProduct: pq.withProduct.Clone(),
...
func (pq *PriceQuery) WithStore(opts ...func(*StoreQuery)) *PriceQuery {
	query := (&StoreClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PriceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Price, error) {
	...
	if query := pq.withProduct; query != nil {
		if err := pq.loadProduct(ctx, query, nodes, nil,
			func(n *Price, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withStore; query != nil {
		if err := pq.loadStore(ctx, query, nodes, nil,
			func(n *Price, e *Store) { n.Edges.Store = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PriceQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Price, init func(*Price), assign func(*Price, *Product)) error {
...
func (pq *PriceQuery) loadStore(ctx context.Context, query *StoreQuery, nodes []*Price, init func(*Price), assign func(*Price, *Store)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Price)
	for i := range nodes {
		fk := nodes[i].StoreID
...
func (pq *PriceQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withProduct != nil {
			_spec.Node.AddColumnOnce(price.FieldProductID)
		}
		if pq.withStore != nil {
			_spec.Node.AddColumnOnce(price.FieldStoreID)
		}
	}
