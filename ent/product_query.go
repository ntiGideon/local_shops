...
func (pq *ProductQuery) QueryStores() *StoreQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, product.StoresTable, product.StoresColumn),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withStores: pq.withStores.Clone(),
...
func (pq *ProductQuery) WithStores(opts ...func(*StoreQuery)) *ProductQuery {
	query := (&StoreClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withPrices; query != nil {
		if err := pq.loadPrices(ctx, query, nodes,
			func(n *Product) { n.Edges.Prices = []*Price{} },
			func(n *Product, e *Price) { n.Edges.Prices = append(n.Edges.Prices, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withStores; query != nil {
		if err := pq.loadStores(ctx, query, nodes,
			func(n *Product) { n.Edges.Stores = []*Store{} },
			func(n *Product, e *Store) { n.Edges.Stores = append(n.Edges.Stores, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadPrices(ctx context.Context, query *PriceQuery, nodes []*Product, init func(*Product), assign func(*Product, *Price)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(price.FieldProductID)
	}
	query.Where(predicate.Price(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.PricesColumn), fks...))
	}))
...
func (pq *ProductQuery) loadStores(ctx context.Context, query *StoreQuery, nodes []*Product, init func(*Product), assign func(*Product, *Store)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Store(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.StoresColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.product_stores
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_stores" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_stores" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
