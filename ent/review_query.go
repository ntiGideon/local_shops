...
func (rq *ReviewQuery) QueryStore() *StoreQuery {
...
func (rq *ReviewQuery) WithStore(opts ...func(*StoreQuery)) *ReviewQuery {
	query := (&StoreClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	...
	if query := rq.withStore; query != nil {
		if err := rq.loadStore(ctx, query, nodes, nil,
			func(n *Review, e *Store) { n.Edges.Store = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReviewQuery) loadStore(ctx context.Context, query *StoreQuery, nodes []*Review, init func(*Review), assign func(*Review, *Store)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Review)
	for i := range nodes {
		fk := nodes[i].StoreID
...
func (rq *ReviewQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withStore != nil {
			_spec.Node.AddColumnOnce(review.FieldStoreID)
		}
	}
